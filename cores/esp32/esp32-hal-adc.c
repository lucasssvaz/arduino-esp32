...
bool analogContinuous(const uint8_t pins[], size_t pins_count, uint32_t conversions_per_pin, uint32_t sampling_freq_hz, void (*userFunc)(void)) {
  ...
  if (alignment_remainder != 0) {
    const uint32_t dma_alignment = ESP_ARDUINO_DMA_BUF_ALIGN;
    uint32_t original_size = adc_handle[adc_unit].conversion_frame_size;
    adc_handle[adc_unit].conversion_frame_size += (dma_alignment - alignment_remainder);
    log_w(
      ...
      original_size, adc_handle[adc_unit].conversion_frame_size, dma_alignment, dma_alignment
    );
  }
