...
.. list-table::
   ...
   * - ``uint64_t``
     - ``"%" PRIu64``
     - **Must** use the PRI macro. Requires full printf (not supported on NEWLIB-nano 32-bit targets).
   * - ``int64_t``
     - ``"%" PRId64``
     - Same rule.
   * - ``size_t``
...
.. code-block:: c
   ...
   // Correct
   log_d("pin=%u addr=0x%04x baud=%" PRIu32 " up=%" PRIu64 " len=%lu err=%d",
         pin, addr, baudrate, uptime, (unsigned long)len, err);

   // Wrong – uses PRIu8/PRIu16 (breaks NEWLIB-nano), %zu (breaks NEWLIB-nano),
   //         and %lu for uint32_t (wrong on PICOLIBC/LP64)
   log_d("pin=%" PRIu8 " addr=0x%04" PRIu16 " baud=%lu up=%" PRIu64 " len=%zu err=%d",
