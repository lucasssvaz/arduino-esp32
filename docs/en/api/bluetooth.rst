...
Disconnects the active SPP session.
...
.. code-block:: cpp
    ...
    BTStatus SerialBT.onConfirmRequest(std::function<void(uint32_t)> callback);

Registers a callback for SSP numeric-comparison pairing. The ``uint32_t`` argument is the numeric value to display to the user. Returns ``BTStatus::OK`` on success.
...
Call from the ``onConfirmRequest`` callback to accept or reject numeric-comparison SSP.

.. code-block:: cpp

    void SerialBT.confirmReply(bool accept);
