...
BLECharacteristic BLEService::createCharacteristic(const BLEUUID &uuid, BLEProperty properties) {
  ...
  // Default permissions based on properties
  BLEPermission perms = static_cast<BLEPermission>(0);
...
std::vector<BLECharacteristic> BLEService::getCharacteristics() const {
  ...
  result.reserve(_impl->characteristics.size());
  for (auto &chr : _impl->characteristics) {
