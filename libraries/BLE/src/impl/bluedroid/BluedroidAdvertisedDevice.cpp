...
// --------------------------------------------------------------------------
// BLEAdvertisedDevice::Impl (shared payload parsing logic)
// --------------------------------------------------------------------------
  ...
  while (pos < len) {
    ...
    switch (adType) {
      ...
      case 0x08: case 0x09:
        ...
        break;
      case 0x0A:
        if (dataLen >= 1) { txPower = static_cast<int8_t>(adData[0]); hasTXPower = true; }
        break;
      case 0x19:
        if (dataLen >= 2) { appearance = adData[0] | (adData[1] << 8); hasAppearance = true; }
        break;
      case 0x16:
        if (dataLen >= 2) {
