...
// Create a characteristic with read/write + authentication properties
BLECharacteristic chr = svc.createCharacteristic(
    CHAR_UUID,
    BLEProperty::Read | BLEProperty::Write
);

// Set permissions to require encryption + authentication for access
chr.setPermissions(BLEPermission::ReadAuthenticated | BLEPermission::WriteAuthenticated);
...
```cpp
// Solution: Use permissions for access control
BLECharacteristic chr = svc.createCharacteristic(
    CHAR_UUID,
    BLEProperty::Read | BLEProperty::Write
);
...
Permissions define the security requirements for accessing an attribute. Combine with bitwise OR.

```cpp
// Basic permissions
BLEPermission::Read                  // Read allowed
BLEPermission::Write                 // Write allowed
...
chr.setPermissions(BLEPermission::ReadEncrypted | BLEPermission::WriteEncrypted);
```
...
chr.setPermissions(BLEPermission::ReadAuthorized | BLEPermission::WriteAuthorized);
```
