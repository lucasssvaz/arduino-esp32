...
- [Key Conceptual Changes](#key-conceptual-changes)
  ...
  - [8. Descriptor Classes Removed](#8-descriptor-classes-removed)
...
In v3.x, properties and permissions were often combined or confused. v4.0 separates them into distinct enums:
...
// v4.0 -- clean separation, same on both stacks
auto chr = svc.createCharacteristic("uuid",
    BLEProperty::Read | BLEProperty::Write);
chr.setPermissions(BLEPermission::ReadEncrypted | BLEPermission::WriteAuthenticated);
```
...
// v4.0 -- services and server both started
svc.start();
server.start();  // NEW: commits GATT database
...
7. **Add `server.start()` after `svc.start()` and before advertising**
...
| `pServer->removeService(svc)` | `server.removeService(svc)` | |
...
| Old (v3.x) | New (v4.0) | Notes |
|---|---|---|
| `pService->createCharacteristic(uuid, props)` → `BLECharacteristic*` | `svc.createCharacteristic(uuid, props)` → `BLECharacteristic` | Value; props use `BLEProperty` |
...
| `hid->startServices()` | `hid.startServices()` | |
...
| `new BLECharacteristic(...)` constructor | `svc.createCharacteristic(uuid, properties)` factory |
...
| BLE5 extended advertising (NimBLE) | `adv.configureExtended(config)`, `adv.startExtended(...)` |
| BLE5 periodic advertising | `adv.configurePeriodicAdv(config)`, `adv.startPeriodicAdv(...)` |
| BLE5 extended scanning | `scan.startExtended(dur, coded, uncoded)` |
...
| Periodic advertising sync | `scan.startPeriodicSync(...)`, `scan.stopPeriodicSync()` |
...
| `BLECharacteristic chr("uuid", props, size)` | `svc.createCharacteristic(BLEUUID("uuid"), props)` | Factory method |
