...
public:
  ...
  BTStatus discoverAsync(std::function<void(const DiscoveryResult &)> callback, uint32_t timeoutMs = 10000);
  ...
  BTStatus onConfirmRequest(std::function<void(uint32_t)> callback);
  BTStatus onAuthComplete(std::function<void(bool)> callback);
  void confirmReply(bool accept);
  ...
  // Data callback
  BTStatus onData(std::function<void(const uint8_t *, size_t)> callback);
